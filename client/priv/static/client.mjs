...
function insert3(map6, key2, value2) {
...
var ServerReturnedAuthenticationResponse = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var UserClickedRegister = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function model_decoder() {
  return field(
    ...
    (variant) => {
      if (variant === "login") {
        return success(new Login(new$()));
      } else if (variant === "logged_in") {
        return field(
          "current_user",
          user_decoder(),
          (current_user) => {
        ...
        return failure(new Login(new$()), "Model");
      }
    }
  );
}
...
var createChildElement = (parent, index5, { key: key2, tag, namespace }) => {
  const node = document2().createElementNS(namespace || NAMESPACE_HTML, tag);
  initialiseMetadata(parent, node, index5, key2);
  return node;
};
var createChildText = (parent, index5, { key: key2, content }) => {
...
var initialiseMetadata = (parent, node, index5 = 0, key2 = "") => {
  const segment = `${key2 || index5}`;
...
function application(init, update3, view2) {
  return new App(init, update3, view2, new$8(empty_list));
}
...
function register_user(user) {
  ...
  let url = "/" + get_api_url(api_register_new_user_path);
  return post(
    url,
    body2,
    expect_json(
...
function decode_registration_data(values3) {
  ...
  return finish(_pipe$2);
}
function user_clicked_register(model, data) {
  let _block;
  if (model instanceof Login) {
    _block = model;
  } else {
  ...
  let model$1 = _block;
  let _block$1;
  let $ = decode_registration_data(data);
  if ($ instanceof Ok) {
    let user = $[0];
    _block$1 = register_user(user);
  } else {
    _block$1 = none2();
  }
  let effect = _block$1;
...
function update2(model, msg) {
  if (msg instanceof ServerRegisteredNewUser) {
    ...
    let data = msg[0];
    return user_clicked_register(model, data);
  }
}
...
function echo$inspectDict(map6) {
  ...
  key_value_pairs.forEach(([key2, value2]) => {
    ...
    body2 = body2 + "#(" + echo$inspect(key2) + ", " + echo$inspect(value2) + ")";
    first2 = false;
  });
  return body2 + "])";
}
...
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 235, end: 343, pattern_start: 246, pattern_end: 261 }
    );
  }
  let json_string = $[0];
  let _block$1;
  let _pipe$1 = parse(json_string, model_decoder());
  _block$1 = unwrap2(_pipe$1, new Login(new$()));
  let initial_model = _block$1;
  let app = application(
    (_) => {
      return [initial_model, none2()];
    },
    update2,
    view
  );
  ...
  if (!($1 instanceof Ok)) {
    throw makeError(
      ...
      "main",
      "Pattern match failed, no pattern matched the value.",
      { value: $1, start: 594, end: 643, pattern_start: 605, pattern_end: 610 }
    );
  }
  return void 0;
}
